...
export default {
  ...
  methods: {
    getOrderConfirm() {
      if (typeof this.cartId === 'string') {
        this.cartId = [parseInt(this.cartId)]
      }
      let obj = {cartIdList: this.cartId}
      orderApi.confirmOrder(obj).then((res) => {
    ...
    subOrder() {
      let obj = {addressId: this.addressId, cartIds: this.cartIds}
